...
public sealed class EfWorkoutRepository	: IWorkoutRepository
{
	...
	public EfWorkoutRepository(LiftrazaDbContext db)
		=> _db = db;
	...
	{
		// SQL aggregation: GROUP BY on the name (converter) with COUNT(*) and MAX(WorkoutDate).
		var rows = await _db.Workouts
			.Where(w => w.UserId == userId)
		...
		var rows = await _db.Workouts
			...
			.SelectMany(w => w.Exercises
				...
				.Select(e => new
				{
					...
					// EF Core does not translate .Value on converted properties inside the query.
