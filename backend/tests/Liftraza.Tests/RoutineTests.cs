...
public class RoutineTests
{
	...
	{
		// The Name value object rejects an empty name before it reaches the entity.
	...
	{
		var routine = new Routine(UserId, new Name("Push"));
		var session = new WorkoutSession(new Name("Day 1"));
	...
	{
		var routine = new Routine(UserId, new Name("Push"));
		routine.AddSession(new WorkoutSession(new Name("Day 1")));
		routine.AddSession(new WorkoutSession(new Name("Day 2")));
		routine.AddSession(new WorkoutSession(new Name("Day 3")));
	...
	public void DuplicateSession_ShouldAppendIndependentCopyWithExercises()
	{
		...
		Assert.Equal("Day 1", copy.Name.Value);
		Assert.Equal(1, copy.DisplayOrder); // second session of the routine
