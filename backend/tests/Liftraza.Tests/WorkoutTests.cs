...
public class WorkoutTests
{
	...
	public void Create_ShouldStoreSnapshotNamesAndDate()
	{
		...
		var workout = new Workout(UserId, new Name("Push / Pull"), new Name("Day 1"), date);

		Assert.Equal(UserId, workout.UserId);
		Assert.Equal("Push / Pull", workout.RoutineName.Value);
		Assert.Equal("Day 1", workout.SessionName.Value);
	...
	public void Create_WithEmptySnapshotNames_ShouldThrow()
	{
		...
		Assert.Throws<ArgumentException>(() => new Workout(UserId, new Name(""), new Name("Day 1"), date));
	...
	{
		var workout = new Workout(UserId, new Name("Push"), new Name("Day 1"), DateTime.UtcNow);
	...
	public void AddSet_WithNegativeWeightOrReps_ShouldThrow()
	{
		...
		// Invariants live in the value objects: negative reps/weight are rejected there.
